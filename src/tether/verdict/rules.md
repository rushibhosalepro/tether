...
| R1 | `DROP`, and a model with an `IN_PRODUCTION` deployment reads the column | **BLOCK** | The feature transform will fail or null out. This is the case the whole project exists for. |
...
| R5 | Reaches a model, but no deployment is `IN_PRODUCTION` | WARN | A real dependency with no live blast radius. Flag it, do not stop the merge. |
